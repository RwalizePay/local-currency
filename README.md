...
```
yarn add @rwalize/local-currency
```

or

```
npm install --save @rwalize/local-currency
```

```javascript
import { Currency } from "@rwalize/local-currency";
...
```javascript
new LocalCurrency.Currency({ amount: 20 }).toString({
